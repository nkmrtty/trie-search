...
class TestTrieSearch(TestCase):
    ...
    def test_search_regexp_splitter(self):
        patterns = [
            "new york",
            "york city",
        ]
        text = "new york, york city and some new york city here"
        splitter = f"[\w\d]+"
        answers = {"new york": {0, 29}, "york city": {10, 33}}
        trie = TrieSearch(patterns=patterns, splitter=splitter)
        for ptn, idx in trie.search_all_patterns(text):
            self.assertIn(ptn, answers)
            self.assertIn(idx, answers[ptn])
